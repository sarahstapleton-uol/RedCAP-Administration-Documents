...
5. An Administrator’s Responsibilities
        ...
        * User provisioning and access controls (via LDAP, local, or SSO) to be managed

        * Audit logs and usage reports are regularly monitored

        * Maintain REDCap and Operating Software Security updates

        * (Typically) Enforce a ‘least amount of viable privileges’ approach that won’t restrict work

        * Coordinate with the Database Administratot for assistance with secure backup procedures and testing that a restored database is performing correctly
        
        * Prior to activating any additional, external modules or an API call, ensure to validate the data, backup and restoration. This step is a critical point in audit logs and should be retained.
